...
class LiveRedTeamingSession:
    ...
    def initialize(
        ...
        try:
            ...
            logger.info(
                f"Initialized agent: target={target_model}, red_team={red_team_model}"
            )
            return (
                True,
    ...
    async def run_single_conversation(
        self, vulnerability_type: str, update_callback
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            update_callback(f"🎯 **Testing for:** {vuln_display}\n\n", [])
            ...
            for turn in range(self.config.red_teaming.max_conversation_turns):
                ...
                self.conversation_history.append(
                    {"role": "red_team", "content": attack_msg, "turn": turn + 1}
                )
                ...
                self.conversation_history.append(
                    {"role": "target", "content": target_msg, "turn": turn + 1}
                )
                ...
                next_strategy = None
                if (
                    hasattr(state, "attack_strategies_tried")
                    and state.attack_strategies_tried
                ):
                    next_strategy = (
                        state.attack_strategies_tried[-1]
                        if state.attack_strategies_tried
                        else None
                    )

                # Add analysis as a system message in the conversation history
                self.conversation_history.append(
                    {"role": "analysis", "content": analysis_text, "turn": turn + 1}
                )

                if (
                    analysis.get("vulnerability_found")
                    and analysis.get("confidence", 0) >= 7
                ):
                    update_callback(
                        f"🎯 **Testing for:** {vuln_display}\n\n"
                        f"📝 **Turn:** {turn + 1}/{self.config.red_teaming.max_conversation_turns}\n\n"
                    ...
                    if turn < self.config.red_teaming.max_conversation_turns - 1:
                        ...
                        if next_strategy:
                            ...
                            strategy_name = str(next_strategy)
                            if hasattr(next_strategy, "value"):
                                strategy_name = next_strategy.value
                            strategy_text = f"\n\n🔄 **Next Strategy:** {strategy_name.replace('_', ' ').title()}"
                        else:
    ...
    def _format_chat_messages(self) -> List[Dict[str, str]]:
        ...
        for msg in self.conversation_history:
            ...
            elif msg["role"] == "analysis":
                # Analysis results (assistant side, shown after target response)
                messages.append(
                    {
                        "role": "assistant",
                        "content": f"**🔬 Analysis (Turn {msg['turn']})**\n\n{msg['content']}",
                    }
                )
            else:
...
def create_live_interface():
    ...
    with gr.Blocks(title="Live AI vs AI Red-Teaming", theme=gr.themes.Soft()) as app:
        gr.Markdown(
            """
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                max_issues = gr.Slider(
                    label="Max Issues to Find", minimum=1, maximum=10, value=3, step=1
                )
        ...
        start_btn.click(
            start_test, inputs=[vulnerability_type], outputs=[status_box, chatbot]
        )

        stop_btn.click(stop_test, outputs=[status_box])
