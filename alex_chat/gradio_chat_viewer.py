...
class RedTeamingViewer:
    ...
    def format_session_overview(self, session: Dict[str, Any]) -> str:
        ...
        overview = f"# 📊 Session Overview\n\n"
        overview += f"**Session:** {session['name']}\n\n"
        overview += f"**Timestamp:** {session['timestamp'].strftime('%Y-%m-%d %H:%M:%S')}\n\n"
        ...
        if session['vulnerabilities']:
            ...
            for i, vuln in enumerate(session['vulnerabilities'], 1):
                ...
                overview += f"{i}. {severity_emoji} **{vuln.get('title', 'Unknown')}**\n"
                overview += f"   - Type: `{vuln.get('type', 'unknown')}`\n"
                overview += f"   - Severity: `{vuln.get('severity', 'unknown').upper()}`\n"
                overview += f"   - Confidence: {vuln.get('metadata', {}).get('confidence', 'N/A')}/10\n\n"
        
        return overview

    def format_vulnerability_details(self, vuln: Dict[str, Any]) -> Tuple[str, str, str]:
    ...
    def _format_analysis_summary(self, vuln: Dict[str, Any]) -> str:
        ...
        analysis += f"- **Conversation Turns:** {metadata.get('conversation_turns', 'N/A')}\n"
...
def create_gradio_interface():
    ...
    with gr.Blocks(
        title="AI vs AI Red-Teaming Viewer",
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                session_dropdown = gr.Dropdown(
                    ...
                    value=viewer.get_session_list()[0] if viewer.get_session_list() else None,
                    interactive=True
                )
        ...
        with gr.Tabs() as tabs:
            with gr.Tab("📋 Details"):
                vulnerability_details = gr.Markdown("Select a vulnerability to view details.")
            ...
            with gr.Tab("🔬 Analysis"):
                analysis_markdown = gr.Markdown("Select a vulnerability to view analysis.")
