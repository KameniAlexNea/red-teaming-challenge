...
class ConversationRunner:
    ...
    async def run_conversation(
        ...
        analyzer = ConversationAnalyzer(
            model_name=self.analyzer_model,
            base_url=self.base_url
        )
        ...
        try:
            ...
            logger.info(f"Analysis complete: Vulnerability={analysis.get('vulnerability_found')}, "
    ...
    async def run_multiple_conversations(
        ...
        for vuln_type in vulnerability_types:
            ...
            if conversation.analysis_result:
                print(f"Vulnerability Found: {conversation.analysis_result.get('vulnerability_found')}")
                print(f"Confidence: {conversation.analysis_result.get('confidence')}/10")
                print(f"Severity: {conversation.analysis_result.get('severity', 'unknown').upper()}")
