...
class TestConversation:
    ...
    def test_conversation_creation(self):
        ...
        assert conv.vulnerability_type is None
        assert conv.attack_strategy is None
        assert conv.success is False
    ...
    def test_conversation_with_vulnerability_type(self):
        ...
        assert conv.vulnerability_type == VulnerabilityType.JAILBREAKING
    ...
    def test_conversation_to_dict(self):
        ...
        assert data["vulnerability_type"] == "prompt_injection"
        assert data["attack_strategy"] == "direct"
    ...
    def test_conversation_to_dict_none_values(self):
        ...
        assert data["vulnerability_type"] is None
...
class TestVulnerability:
    ...
    def test_vulnerability_creation(self):
        ...
        assert vuln.type == VulnerabilityType.DECEPTIVE_ALIGNMENT
        ...
        assert vuln.attack_vector == "Test attack"
        assert vuln.conversation_id == "conv-456"
        assert vuln.proof_of_concept == "Proof of concept"
        assert vuln.mitigation_suggestions == []
    ...
    def test_vulnerability_with_mitigations(self):
        ...
        assert vuln.mitigation_suggestions == mitigations
    ...
    def test_vulnerability_to_dict(self):
        ...
        assert data["id"] == "vuln-serialize"
        assert data["type"] == "data_extraction"
        assert data["severity"] == "high"
        assert data["title"] == "Data Extraction Test"
        assert data["description"] == "Test vulnerability for data extraction"
        assert data["attack_vector"] == "Malicious prompt"
        assert data["conversation_id"] == "conv-serialize"
        assert data["proof_of_concept"] == "Successfully extracted data"
        assert data["mitigation_suggestions"] == [
            "Input validation",
            "Output filtering",
        ]
...
class TestRedTeamingState:
    ...
    def test_red_teaming_state_creation(self):
        ...
        assert state.discovered_vulnerabilities == []
        assert state.failed_attempts == []
        assert state.current_vulnerability_type is None
        assert state.conversation_turn == 0
        assert state.max_turns == 10
        assert state.attack_strategies_tried == []
    ...
    def test_issues_found_property(self):
        ...
        state.discovered_vulnerabilities.append(vuln1)
        assert state.issues_found == 1

        state.discovered_vulnerabilities.append(vuln2)
        assert state.issues_found == 2
    ...
    def test_red_teaming_state_to_dict(self):
        ...
        assert isinstance(data["current_conversation"], dict)
        assert data["current_conversation"]["id"] == "test-conv"
        assert len(data["discovered_vulnerabilities"]) == 1
        assert isinstance(data["discovered_vulnerabilities"][0], dict)
        assert len(data["failed_attempts"]) == 1
        assert isinstance(data["failed_attempts"][0], dict)
        assert data["current_vulnerability_type"] == "bias_amplification"
        assert data["conversation_turn"] == 3
        assert data["max_turns"] == 5
        assert data["attack_strategies_tried"] == ["direct", "indirect"]
    ...
    def test_red_teaming_state_to_dict_none_values(self):
        ...
        assert data["current_vulnerability_type"] is None
...
class TestAttackPrompt:
    ...
    def test_attack_prompt_creation(self):
        ...
        assert prompt.content == "Test attack prompt"
        assert prompt.vulnerability_type == VulnerabilityType.HARMFUL_CONTENT_GENERATION
        assert prompt.strategy == "direct_approach"
        assert prompt.expected_outcome == "Model generates harmful content"
        assert prompt.follow_up_prompts == []
    ...
    def test_attack_prompt_with_follow_ups(self):
        ...
        assert prompt.follow_up_prompts == follow_ups
    ...
    def test_attack_prompt_to_dict(self):
        ...
        assert data["content"] == "Complex attack prompt"
        assert data["vulnerability_type"] == "privacy_violations"
        assert data["strategy"] == "social_engineering"
        assert data["expected_outcome"] == "Extract private information"
        assert data["follow_up_prompts"] == ["Escalation 1", "Escalation 2"]
        assert data["metadata"] == metadata
...
class TestDataclassIntegration:
    ...
    def test_conversation_with_vulnerability_workflow(self):
        ...
        assert state_data["issues_found"] == 1
        assert state_data["current_vulnerability_type"] == "reward_hacking"
        assert vuln_data["type"] == "reward_hacking"
        assert vuln_data["severity"] == "critical"
        assert conv_data["vulnerability_type"] == "reward_hacking"
    ...
    def test_serialization_roundtrip_compatibility(self):
        ...
        assert conv_dict["id"] == "test"
        assert vuln_dict["id"] == "vuln1"
        assert state_dict["issues_found"] == 1
        assert prompt_dict["content"] == "Test"
